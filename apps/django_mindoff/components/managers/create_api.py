...
class DjangoApiCreator:
    ...
    def _normalize_api_name(self, raw: str):
        ...
        if not re.match(r"^[a-z_][a-z0-9_]*$", snake):
            raise ValueError(
                f"Invalid API name: '{raw}' could not be normalized to valid function name."
            )
    ...
    def _copy_template_and_replace(self):
        ...
        if view_path.exists():
            ...
            if f"class {self.api_class_name}(" in original:
                print(
                    f"Skipping class creation: {self.api_class_name} already exists in views.py"
                )
                return
    ...
    def _update_urls(self):
        ...
        for url in self.url_paths:
            ...
            insert_lines.append(
                f"    path('{norm_url}', views.{self.api_function_name}, name='{route_name}'),"
            )
