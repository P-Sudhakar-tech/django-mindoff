...
class DjangoProjectCreator:
    ...
    def _prompt_optional_dependencies(self):
        optional = []
        if input("Install factory_boy for Tests? (y/n): ").lower() == "y":
            optional.append("factory_boy")
        if input("Install model_bakery for Tests? (y/n): ").lower() == "y":
            optional.extend(["model-bakery", "Faker"])
        if input("Install polars? (y/n): ").lower() == "y":
            optional.extend(["polars", "numpy"])
        return optional
