...
class MindoffRouterTestCase:
    """
    Base class for version-router tests generated by the ``createapi`` command.
    Mirrors the structure of ``MindoffTestCase`` — wire-up happens in the
    ``run`` autouse fixture, all test methods are inherited.

    Subclasses declare two class attributes (set by the code generator):

        app_module           : str  – dotted path to the app's views module
                                      e.g. "apps.inventory.views"
        router_function_name : str  – name of the router instance in that module
                                      e.g. "create_inventory_router"

    All version-mapping assertions are derived dynamically from the router's
    VERSION_MAP at test time, so they remain correct as versions are added later.
    """
    ...
    # ------------------------------------------------------------------
    # Version-map integrity: every registered version dispatches correctly
    # ------------------------------------------------------------------
    ...
    def test_every_version_dispatches_to_correct_class(self):
        ...
        not to a sibling version or an unrelated view.
        """
        ...
        for version, expected_class in self.version_map.items():
            ...
            assert response.status_code == 200, (
                f"Version {version} → {expected_class.__name__} returned "
                f"HTTP {response.status_code}, expected 200."
            )

    def test_no_version_is_mapped_to_wrong_class(self):
        """
        Guards against copy-paste errors in VERSION_MAP (e.g. {1: V2, 2: V2}).
        Each version must map to a distinct class.
        """
        classes = list(self.version_map.values())
        versions = list(self.version_map.keys())
        for i, (version, expected_class) in enumerate(self.version_map.items()):
            sibling_classes = [c for j, c in enumerate(classes) if j != i]
            for sibling in sibling_classes:
                assert sibling is not expected_class, (
                    f"Version {version} and version {versions[classes.index(sibling)]} "
                    f"both map to {expected_class.__name__}. Each version must map to "
                    f"a distinct class."
                )

    def test_version_map_is_not_empty(self):
        """VERSION_MAP must contain at least one entry."""
        assert (
            len(self.version_map) > 0
        ), f"Router '{self.router_function_name}' has an empty VERSION_MAP."

    def test_version_map_contains_version_1(self):
        """VERSION_MAP must always register version 1 as the base version."""
        assert 1 in self.version_map, (
            f"Router '{self.router_function_name}' VERSION_MAP is missing version 1. "
            f"Registered versions: {list(self.version_map.keys())}"
        )

    def test_all_version_keys_are_integers(self):
        """All VERSION_MAP keys must be integers (1, 2, 3 …)."""
        non_int = [k for k in self.version_map if not isinstance(k, int)]
        assert not non_int, (
            f"Non-integer version keys found in VERSION_MAP: {non_int}. "
            f"Version keys must be integers."
        )

    def test_all_version_values_are_classes(self):
        """All VERSION_MAP values must be classes (not instances or None)."""
        import inspect

        non_class = {
            k: v for k, v in self.version_map.items() if not inspect.isclass(v)
        }
        assert not non_class, (
            f"Non-class values found in VERSION_MAP: "
            f"{', '.join(f'version {k}: {v!r}' for k, v in non_class.items())}. "
            f"VERSION_MAP values must be APIView classes."
        )

    # ------------------------------------------------------------------
    # Sad-path: unknown / missing version
    # ------------------------------------------------------------------

    def test_unknown_version_returns_404(self):
        """A version absent from VERSION_MAP must return HTTP 404."""
        from django.test import RequestFactory
    ...
    def test_unknown_version_response_body(self):
        """404 body must contain both 'detail' and 'available_versions' keys."""
        import json
        from django.test import RequestFactory
    ...
    def test_unknown_version_lists_registered_versions(self):
        """
        The 'available_versions' in the 404 body must exactly match
        the keys registered in VERSION_MAP — no more, no less.
        """
        import json
        from django.test import RequestFactory
        ...
        assert set(body["available_versions"]) == set(self.version_map.keys()), (
            f"available_versions in 404 response {body['available_versions']} "
            f"does not match VERSION_MAP keys {list(self.version_map.keys())}."
        )

    def test_missing_version_kwarg_returns_404(self):
        """Router called with no version kwarg at all must return HTTP 404."""
        from django.test import RequestFactory
